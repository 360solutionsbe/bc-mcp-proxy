...
- A small open-source AL companion app can close the gap: an API page that reports the signed-in user's effective Read/Insert/Modify/Delete per API page (`RecordRef.ReadPermission` / `WritePermission`) lets the proxy hide exactly the right tools with one call, including write tools, and without probing. This is planned as `bc-mcp-guard`.
