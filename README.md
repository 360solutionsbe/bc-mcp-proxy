...
- **`HTTP 400 Bad Request` from `mcp.businesscentral.dynamics.com`.** Sign-in worked; BC is rejecting the request *routing*, not the token. Almost always: a wrong **Environment name** (exact, case-sensitive), a missing/incorrect **MCP Configuration Name** (required when a named MCP configuration exists — and easy to leave blank in the extension settings since it's not shown in Claude Desktop's args log), a wrong **Company**, or the signed-in account lacking access to that environment. The proxy now surfaces this as a single actionable line in the log (echoing your effective Environment/Company/ConfigurationName) and **stays alive** instead of crash-looping with a Python traceback — fix the field and restart.
